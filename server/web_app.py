...
def _parse_users(env_value: str) -> dict:
    """Parse 'user1:pass1,user2:pass2' into {user: pass}. Whitespace tolerant."""
    users = {}
    if not env_value:
        return users
...
# Prefer the multi-user env var; fall back to the single-pair env vars
# for backward compatibility.
USERS = _parse_users(os.getenv("VO_DASHBOARD_USERS", ""))
if not USERS:
    _single_user = os.getenv("VO_DASHBOARD_USER", "")
    _single_pass = os.getenv("VO_DASHBOARD_PASSWORD", "")
    if _single_user and _single_pass:
        USERS = {_single_user: _single_pass}
...
def _check_auth(auth_header: str) -> bool:
    ...
    try:
        ...
        return expected is not None and expected == pw
    except Exception:
        return False
